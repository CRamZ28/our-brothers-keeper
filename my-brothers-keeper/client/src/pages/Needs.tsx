            ...
            {completedNeeds.length === 0 ? (
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {completedNeeds.map((need) => {
                  const Icon = categoryIcons[need.category];
                  return (
                    <GlassCard key={need.id} className="hover-lift flex flex-col h-full opacity-75">
                      <CardHeader className="pb-4 pt-6">
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex items-start gap-3 flex-1 min-w-0">
                            <div className="w-12 h-12 rounded-lg bg-white/30 flex items-center justify-center shrink-0">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1 flex-wrap">
                                <span className="text-xs font-medium text-foreground/70 uppercase tracking-wide">{need.category}</span>
                                <Badge variant="outline" className="bg-gray-400/30 text-foreground border-gray-400/50">
                                  Completed
                                </Badge>
                                {need.priority === "urgent" && (
                                  <Badge variant="outline" className="bg-red-100/80 text-red-700 border-red-300/50 text-xs">
                      ...
                      <CardContent className="space-y-4 pt-0 flex-grow">
                        {need.dueAt && (
                          <div className="flex items-center gap-2 text-sm text-foreground/70">
                            <Calendar className="w-4 h-4" />
                            <span className="font-medium">Due: {formatDateWithDayOfWeek(new Date(need.dueAt))}</span>
                          </div>
                        )}
                        {need.details && (
                          <p className="text-sm text-gray-600 leading-relaxed">{need.details}</p>
