...
// Auth.js: Session storage table
// Schema required by @auth/drizzle-adapter — see https://authjs.dev/getting-started/adapters/drizzle
 ...
 * Uses varchar IDs generated by Auth.js. The `name`, `email`, `emailVerified`,
 * and `image` columns are required by @auth/drizzle-adapter. Everything else
 * is OBK-specific (role, tier, household, etc).
 */
...
/**
 * Auth.js accounts table — required by @auth/drizzle-adapter even for
 * email-only providers. Stores OAuth tokens when social providers are added.
 */
...
/**
 * Auth.js verification tokens — stores the random tokens emailed in magic links.
 * Auth.js consumes the row when the user clicks the link.
 */
